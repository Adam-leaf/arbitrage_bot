...
def calc_swap(base_price_luna_virtual, sol_price_luna_solana, base_usd_price, solana_usd_price, trade_action, target_close = TARGET_CLOSE):
    ...
    logger.info(f"\n\n=== Initial Parameters ===\n"
    f"Base Price LUNA/VIRTUAL: {base_price_luna_virtual:.6f}\n"
    f"Sol Price LUNA/SOLANA: {sol_price_luna_solana:.6f}\n"
    ...
    f"Target Gap After Trade: ${target_gap:.8f}\n"
    ...
    # When base_price < solana_price
        target_base_price = base_usd_price + price_move
        target_sol_price = solana_usd_price - price_move
        
        base_luna_amount = base_pool_usd_depth * (price_move / base_usd_price**2)
        sol_luna_amount = sol_pool_usd_depth * (price_move / solana_usd_price**2)

        price_impact_base = target_base_price/base_usd_price * 100 - 100
        price_impact_sol = target_sol_price/solana_usd_price * 100 - 100
        
        logger.info(f"\n=== Required Moves ===\n"
                    f"Base: Need to BUY {base_luna_amount:.4f} LUNA to move price "
                    f"\nPrice Impact: {price_impact_base}"
                    f"${base_usd_price:.6f} -> ${target_base_price:.6f}"
                    f"\nSol: Need to SELL {sol_luna_amount:.4f} LUNA to move price "
                    f"${solana_usd_price:.6f} -> ${target_sol_price:.6f}"
                    f"\nPrice Impact: {price_impact_sol}")
        
    else:  # buy_sol_sell_base
        # When solana_price < base_price
        target_sol_price = solana_usd_price + price_move
        target_base_price = base_usd_price - price_move
        ...
        logger.info(f"\n=== Required Moves ===\n"
                   f"Base: Need to SELL {base_luna_amount:.4f} LUNA to move price "
                   f"\nPrice Impact: {price_impact_base} "
                   f"${base_usd_price:.4f} -> ${target_base_price:.4f}"
                   f"\nSol: Need to BUY {sol_luna_amount:.4f} LUNA to move price "
                   f"${solana_usd_price:.4f} -> ${target_sol_price:.4f}\n"
                   f"\nPrice Impact: {price_impact_sol}")
    

    # Take the smaller amount to ensure balanced swap
    luna_amount = min(base_luna_amount, sol_luna_amount)
    ...
    base_usd_amount = luna_amount * base_usd_price  # could also use solana_usd_price
    ...
    logger.info(f"\n=== Arbitrage Calculation ===\n"
                ...
                f"LUNA Amount: {luna_amount:.6f}\n"
                f"BASE - USD Value: ${base_usd_amount:.2f}\n"
                f"SOL - USD Value: ${sol_usd_amount:.2f}\n"
                f"Expected Profit: ${expected_profit_usd:.2f}")
